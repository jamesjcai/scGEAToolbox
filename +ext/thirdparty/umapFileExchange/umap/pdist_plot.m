...
%     run_umap_main previously produced.
%
...
if template_applied
    embedding = run_umap_main(csv_file_or_data, 'template_file', template_file, 'verbose', verbose);
elseif supervised
    sample_hi(:, label_column) = [];
    embedding = run_umap_main(csv_file_or_data, 'label_column', label_column, 'verbose', verbose);
else
    embedding = run_umap_main(csv_file_or_data, 'verbose', verbose);
end
...
if vary_dims && n_dims > 2
    ...
    for d = 3:maxD
        if template_applied
            embedding = run_umap_main(csv_file_or_data, 'template_file', template_file, 'verbose', verbose, 'n_components', d);
        elseif supervised
            embedding = run_umap_main(csv_file_or_data, 'label_column', label_column, 'verbose', verbose, 'n_components', d);
        else
            embedding = run_umap_main(csv_file_or_data, 'verbose', verbose, 'n_components', d);
        end
