...
axPosN = get(hAxes, 'Position');
axOPN = get(hAxes, 'OuterPosition');
set(hAxes, 'Units', 'Pixels');
axPosP = get(hAxes, 'Position');
axOPP = get(hAxes, 'OuterPosition');
...
if ~isempty(axInfo)
    ...
    if isempty(axInfo.xlab), i = int2str(pos(1));
    else, i = axInfo.xlab{pos(1)};
    end
    if isempty(axInfo.ylab), j = int2str(pos(2));
    else, j = axInfo.ylab{pos(2)};
    end
