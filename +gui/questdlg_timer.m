function ButtonName = questdlg_timer(TimeOutValue, Question, Title, Btn1, Btn2, Btn3, Default)
    ...
    error('MATLAB:questdlg_timer:WrongNumberOutputs', 'Wrong number of output arguments for QUESTDLG_TIMER');
        ...
        if nargin < 2
            error('MATLAB:questdlg_timer:TooFewArguments', 'Too few arguments for QUESTDLG_TIMER');
                ...
                if nargin <= 4, Btn1 = 'Yes';
                    Btn2 = 'No';
                    Btn3 = 'Cancel';
                    NumButtons = 3;
                end
                if nargin == 5, Default = Btn2;
                    Btn2 = [];
                    Btn3 = [];
                    NumButtons = 1;
                end
                if nargin == 6, Default = Btn3;
                    Btn3 = [];
                    NumButtons = 2;
                end
                if nargin == 7, NumButtons = 3; end
                if nargin > 7
                    error('MATLAB:questdlg_timer:TooManyInputs', 'Too many input arguments');
                    NumButtons = 3; %#ok
                end
