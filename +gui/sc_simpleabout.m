...
if ~done
    hFig = figure('MenuBar','none','ToolBar','none', ...
        ...
        'DockControls','off', ...
        'Resize','off', ...
        'WindowButtonDownFcn',@(src, ~) i_close_about_dialog(src, parentfig),...
        'WindowKeyPressFcn',@(src, ~) i_close_about_dialog(src, parentfig));
end
...
hFig.Visible=true;
hFig.CloseRequestFcn = @(src,~) i_close_about_dialog(src, parentfig);
...
function i_close_about_dialog(hFig, parentfig)
