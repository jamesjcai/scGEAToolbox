classdef Library
    ...
    methods (Static)
        ...
        function [x, y, click, fig, ax1, ax2] = UiMakeDecision(rho, delta, varargin)
            ...
            title(ax1, 'Decision Graph', 'FontSize', 15);
            xlabel(ax1, '\rho');
            ylabel(ax1, '\delta');
            grid(ax1, 'on');
            %Show hints
        ...
        function [fig, ax] = UiPlotGamma(gamma, varargin)
            ...
            pbaspect(ax, [1, 1, 1]);
            title(ax, 'Distribution of \gamma value', 'FontSize', 15);
            xlabel(ax, 'Number');
            ylabel(ax, '\gamma');
            grid(ax, 'on');
        end
        ...
        function [fig, ax] = UiPlotResultAndRho(cluster, center, rho, data, cmap, varargin)
            ...
            pbaspect(ax, [1, 1, 1]);
            title(ax, 'Result and \rho value (3D)', 'FontSize', 15);
            xlabel('X');
            ylabel('Y');
            zlabel('\rho')
            xlabel('X');
        ...
        function [fig, ax] = UiPlotResultAndDelta(cluster, center, delta, data, cmap, varargin)
            ...
            pbaspect(ax, [1, 1, 1]);
            title(ax, 'Result and \delta value (3D)', 'FontSize', 15);
            xlabel('X');
            ylabel('Y');
            zlabel('\delta');
            xlabel('X');
