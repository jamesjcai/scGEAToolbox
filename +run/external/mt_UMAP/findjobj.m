...
% $Revision: 1.2 $  $Date: 2020/02/07 07:00:36 $
...
try
    ...
    if nargin
        ...
        elseif ishghandle(container) % && ~isa(container,'java.awt.Container')
            ...
            try hJavaFrame = get(hFig, 'JavaFrame');
            catch, hJavaFrame = [];
            end
            warning(oldWarn);
    ...
    function warnInvisible(varargin)
        ...
        try
            ...
            function traverseContainer(jcontainer, level, parent)
                ...
                try newHandle = handle(jcontainer, 'callbackproperties');
                catch, newHandle = handle(jcontainer);
                end
                try handles(thisIdx) = newHandle;
                catch, handles = newHandle;
                end
                try
                ...
                while ~isempty(varargin) && ~isempty(handles)
                    ...
                    switch lower(varargin{1})
                        ...
                        case 'position'
                            ...
                            if invertFlag, foundIdx = ~foundIdx;
                                invertFlag = false;
                            end
                        case 'size'
                                                        ...
                                                        function [cbData, cbHeaders, cbTableEnabled] = getCbsData(obj, stripStdCbsFlag)
                                                            ...
                                                            try
                                                                ...
                                                                if ~isempty(cbNames)
                                                                    ...
                                                                    hgHandleFlag = 0;
                                                                    try hgHandleFlag = ishghandle(obj);
                                                                    catch, end %#ok
                                                                    try
