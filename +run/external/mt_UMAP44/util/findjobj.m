...
% $Revision: 1.5 $  $Date: 2022/03/26 00:50:42 $
    ...
    try
        ...
        if nargin
            ...
            elseif isequal(container,0)  % root
                ...
                container = com.mathworks.mde.desk.MLDesktop.getInstance.getMainFrame; %#ok<*JAPIMATHWORKS> 
            ...
            elseif ishghandle(container) % && ~isa(container,'java.awt.Container')
                ...
                try hJavaFrame = get(hFig,'JavaFrame'); catch, hJavaFrame = []; end %#ok<*JAVFM> 
                warning(oldWarn);
    ...
    function setProp(list,name,value,category)
        prop = eval('com.jidesoft.grid.DefaultProperty();');  %#ok<*EVLCS> % prevent JIDE alert by run-time (not load-time) evaluation
        prop.setName(name);
...
%}
        ...
        lowerPanel.add(createJButton('Check for<br>updates',   {@btCheckFex_Callback}, 'Check the MathWorks File Exchange for the latest version of FindJObj'));
        ...
        [obj, hcontainer] = javacomponent(globalPanel, [0,0,pos(3:4)], hTreeFig); %#ok<*JAVCM> 
        warning(oldWarn2);
...
%}
    ...
    function btCheckFex_Callback(src, evd, varargin)  %#ok
        try
            % Check the File Exchange for the latest version
