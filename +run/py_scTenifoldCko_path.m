...
if nargin < 5 || isempty(targetpath), targetpath=sprintf('%s+%s', celltype1, celltype2); end
...
if targetpath==sprintf('%s+%s', celltype1, celltype2)
    sce2.X(sce2.g==targetg, :)=0;
elseif targetpath==celltype1
    sce2.X(sce2.g==targetg, sce2.c_cell_type_tx==celltype1)=0;
elseif targetpath==celltype2
...
% --------------------------------------------------
    ...
    function in_prepareA12(sce, targetg)
        ...
        if contains(targetpath, celltype1)
            fprintf('\nKO gene in %s.\n', celltype1);
            if nnz(A(idx, :) ~= 0) < 10
        ...
        if contains(targetpath, celltype2)
            fprintf('\nKO gene in %s.\n', celltype2);
            if nnz(A(idx, :) ~= 0) < 10
                warning('KO gene (%s) has no link or too few links with other genes.', ...
                    targetg);
