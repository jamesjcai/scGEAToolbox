function [res_pos,res_neg,res_descr,p_gene] = MrGSEA(data,group,data_entrez,file_name,opts)
...
ind_d = cell(1,GS.nb);  %indices of all GS genes in dataset
RI = ind_d;
for a=1:GS.nb
    [common,ind_d{a}] = intersect_fast(data_entrez,GS.entrez{a});  %find genes that are in given GS
...
res_descr={'GS.NAME','GS size','ES','ES_p-val','NES','NES_p-val','NES_q-val','NES_FWER','# genes in LEF','LEF genes'};

if opts.save
    warning off
    
    % save results in .mat file
    save([file_name '.mat'],'res_neg','res_pos','res_descr','LEF_neg','LEF_pos')
