...
GOMF <- gmtPathways('https://amp.pharm.mssm.edu/Enrichr/geneSetLibrary?mode=text&libraryName=GO_Molecular_Function_2018')
...
set.seed(1)
eGOMF <- fgseaMultilevel(GOMF, Z, scoreType = "pos")
set.seed(1)
...
eGOMF <- eGOMF[eGOMF$ES > 0 & eGOMF$padj < 0.05,]
eREACTOME <- eREACTOME[eREACTOME$ES > 0 & eREACTOME$padj < 0.05,]

E <- do.call(rbind.data.frame, list(eKEGG, eBIOP, eGOBP, eGOMF, eREACTOME))
