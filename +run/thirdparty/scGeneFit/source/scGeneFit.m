...
% 
% -------------------------------------------------------------------------
function [markers, projected_data]=scGeneFit(data, labels, num_markers, opt)
...
[samples, sublabels]=subsample(data, labels{1}, samples_fraction(1) );
[Delta,smallest]=select_constraints(samples, sublabels, constraints_neighbors);
...
[subsamples, sublabels]=subsample(data(:,indices), sublabels, samples_fraction(level) );
[D, s]=select_constraints(subsamples, sublabels, constraints_neighbors);
...
size(Delta)
M=sqz_hinge(Delta, hinge_scale*smallest, num_markers);
