function callback_RunEnrichr(app, ~, predefinedlist, enrichrtype, ...
...
if app.sce.NumGenes == 0
    askbackground = false;
end

    [FigureHandle, sce] = xui.gui_getfigsce(app);
...
%        '', {options{1}, options{2}}, options{1});
    ...
    switch answer
        ...
        case options{2}
            if height(T) > 1
                gui.callback_EnrichrTab2Circos(app, [], T);
            else
