...
%   License: BSD 3 clause
    ...
    methods
        ...
        function this=Supervisors(labels, labelMap, embedding, ...
                ax, fromD, args)
            if nargin<6
                args=struct();
            end
            ...
            if isstruct(args)
                if isfield(args, 'maxDeviantParameters')
                    this.maxDeviantParameters=args.maxDeviantParameters;
                end
                if isfield(args, 'robustConcordance')
                    this.robustConcordance=args.robustConcordance;
                end
                if isfield(args, 'rescale') 
                    if args.rescale~=1
                        this.embedding=this.embedding*args.rescale;
                    end
                end
            end
        end
                ...
                if id>0      
                    if isSigId(i)
                        ...
                        if isempty(clr_)
                            ...
                            clrs(sig_idx,:)=str2double(clr_)/256; 
                        end
                    else
    ...
    methods(Access=private)
        ...
        function qf=qfMatchWithClusters(this, data, dns, numClusters, ...
            ...
            qf=run_HiD_match(this.embedding(:,1:D), ...
                ...
                'matchStrategy', matchStrategy, 'log10', false, ...
                'pu', pu2, 'maxDeviantParameters', ...
                this.maxDeviantParameters, 'robustConcordance', ...
                this.robustConcordance);
            qf.tClrs=clrs;
    ...
    methods
        ...
        function resolveTestDataMatching(this, testSetData, ...
                matchType, pu, confidenceLevel)
            if ~any(matchType==4) && isempty(this.mlp_model)
                return;
            end
            ...
            if ~isempty(this.mlp_model)
                if nargin>=5 && ~isempty(confidenceLevel)
                    this.mlp_confidence_level=confidenceLevel;
                end
                if isempty(this.mlp_labels) %idempotent
                    try
                        if ~isempty(this.mlp_supervise_labels)
                            this.mlp_labels=this.mlp_supervise_labels;
                            this.mlp_supervise_labels=[];
                        else
                            if ~isempty(pb)
                                %always use full one first
                                [this.mlp_labels, confidence]...
                                    =this.predict(testSetData, pu, false);
                                if isempty(this.mlp_labels)
                                    [this.mlp_labels, confidence]...
                                        =predict(testSetData, pu, true);
                                end
                            else
                                [this.mlp_labels, confidence]=...
                                    this.predict(testSetData, pu, []);
                            end
        ...
        function labels=getLabels(this, pb)
            if ~isempty(pb)
                labels=pb.decompress(this.labels);
            else
                labels=this.labels;
            end
        end

        function [labels, confidence]=predict(this, testSetData, pu, fast)
             if isempty(fast)
                file=this.mlp_model;
            else
        ...
        function ok=predictMlp(this, testSetData, pu)
            ok=false;
            if ~isempty(this.mlp_model) ...
                    && isempty(this.mlp_labels) 
                try
                    if isa(testSetData, 'SuhProbabilityBins')
                        testSetData=testSetData.compress;
                        %always use full one first
                        [this.mlp_supervise_labels, confidence]=...
                            this.predict(testSetData, pu, false);
                        if isempty(this.mlp_supervise_labels)
                            [this.mlp_supervise_labels, confidence]=...
                                this.predict(testSetData, pu, true);
                        end
                    else
                        [this.mlp_supervise_labels, confidence]=...
                            this.predict(testSetData, pu, []);                    
        ...
        function [qf, qft]=qfTreeSupervisors(this, visibleOrLocateFig, pu, ttl, askTitle)
            if nargin<5
                askTitle=false;
                if nargin<4
        ...
        function [testSet, lbls,qfForClusterMatch]=plotTestSet(this, ...
            ...
            if good
                ...
                if ~isempty(this.mlp_supervise_labels)
                    lbls=this.mlp_supervise_labels;
                end
                if doJavaLegend 
        ...
        function [qf, qft]=qfTreeSupervisees(this, embedding, rawData, ...
                visibleOrLocateFig, pu, ttl, askTitle)
            if nargin<7
                askTitle=false;
                if nargin<6
        ...
        function Hs=drawClusterBorders(this, ax)
            Hs=[];
            ...
            for i=1:N_
                ...
                Hs(i)=gridEdge(this.density, true, i, clr, ax, ...
                    .8, '.', '-', .5);
                if Supervisors.VERBOSE
