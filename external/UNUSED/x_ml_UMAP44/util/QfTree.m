...
%   https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6586874/
    ...
    methods
        ...
        function setGt(this, gt, gid)
            ...
            ToolBarMethods.addButton(tb_,'heatMapHot.png', ...
        ...
        function optionsMenu(this, idx)
            ...
            if idx<1
                ...
                Gui.NewMenuItem(this.mnuOpts, ...
                    ...
                    @(h,e)showMarkerHeatMap(this, false), 'heatMapHot.png');
        ...
        %used by heat map
        function rowClicked(this, ~, row)
            feval(this.mouseEar, -1, row);
        end
        
        %used by heat map
        function cellClicked(this, ~, row, ~)
            feval(this.pathFinder, row, 'south west+');
        end

        %used by heat map
        function columnClicked(this, ax, column)
            fprintf('No SuhHeatMap column listener column=%d/%d, ax=%d\n',...
                column, size(this.qf.measurements, 2), ~isempty(ax));
        end

        %used by heat map
        function [tip, botPnl]=rowHeaderEntered(this, ~, row)
            [tip, botPnl]=feval(this.mouseEar, true, row);
        end
        ...
        function showMarkerHeatMap(this, justBrowse, phenogramOrientation)
            try
                ...
                args.rowClickedCallback=@(ax, row)...
                    rowClicked(this,ax,row);
                args.columnClickedCallback=@(ax,column)...
                    columnClicked(this,ax,column);
                args.cellClickedCallback=@(ax,row,column)...
                    cellClicked(this, ax, row, column);
                args.rowHeaderEnteredCallback=@(ax,row)...
                    rowHeaderEntered(this, ax, row);
                this.pathFinder=@pathFinder;
                this.mouseEar=@mouseEar;
            ...
            function [html, nCols]=topHtml
                ...
                htmlHeat=SuhHeatMap.Html(args, this.ax, ...
            ...
            function setMap
                ...
                maxN=length(I);
                [~, tp1]=SuhHeatMap.Html(args, this.ax, ...
        ...
        function I=sortXyAndAx(this, distFromWhere, which, rowsIdxs)
            ...
            elseif startsWith(distFromWhere, 'Highest') ||...
                ...
                I=SuhHeatMap.SortByMeasurement(distFromWhere, this.heatMapArgs);                
            else
                I=SuhHeatMap.SortDists(this.ax, xy, distFromWhere);
            end
    ...
    methods(Static)
        ...
        function [fig, ax, nodes,xyNormalized, figs1D, nodeNames, ...
            ...
            op=str2double(props.get(prop,'0'));  
            if length(op)==4
