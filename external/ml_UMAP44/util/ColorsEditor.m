...
%   License: BSD 3 clause
    ...
    methods
        function this=ColorsEditor(inNames, inColors, itemName, ...
            ...
            for i=1:R
                ...
                if nonDflts.contains(key)
                    warning('Case insensitive duplicate %s', char(key));
                else
                    nonDflts.add(key);
                    names{end+1}=inName;
                ...
                if ~hideDflts
                    ...
                    while it.hasNext
                        ...
                        if ~nonDflts.contains(java.lang.String(lower(k)))
                            ...
                            try
                                this.colors(end+1,:)=str2double(color);  
            ...
            this.table.initContiguousSelections;
            if visible
        ...
        function showTip(this, tip)
            ...
            this.app.showToolTip(this.btnChangeDflts, tip, ...
                xTip, yTip, 10, [], false, .31);
    ...
    methods(Access=private)        
        function initBtns(this)
            ...
            [~,H]=javacomponent(Gui.CheckBox(...
                ...
                @(h,e)hearAlwaysRefresh(this, h), 'Select to auto-refresh')); %#ok<*JAVCM> 
        ...
        function useDflts(this)
            ...
            for i=1:N_
                mr=mrs(i);
                name=this.names{mr};
                dfltColor=str2double(this.getDflt(name));  
                vr=ColorsEditor.GetVisualRow(J, name);
        ...
        function pick(this, ~, E)
            ...
            if nargin>2 
                if isa(E, 'matlab.ui.eventdata.CellEditData')
                    % uitable CellSelectionCallback works!!
                    % thus treat scatter column selection as pick button
    ...
    methods(Access=private)
        function initSlider(this)
            fig_=this.table.fig;
            bgc=fig_.Color .*.91;
            u=uipanel(fig_, 'units', 'normalized', ...
                'position', [.42 0 .58 .08], ...
        ...
        function sldChanged(this)
            ...
            if ColorsEditor.DEBUG_SLD
                [this.sldClrs nan(cnt,1) this.sldClrsStart nan(cnt, 1)...
                    this.sldClrsEnd nan(cnt,1) clrs] %#ok<NOPRT> 
            end
            for i=1:cnt
        ...
        function updateSlider(this, mrs, vrs)
            ...
            if cnt>0
                cannotShade=any(clrs==0, 2) & any(clrs==1, 2);
                if any(cannotShade)
            ...
            if cnt==1
                ...
                [~,iMn]=min(sum(this.sldClrsStart, 2));
                startClr=this.sldClrsStart(iMn,:);
                [~,iMx]=max(sum(this.sldClrsEnd, 2));
