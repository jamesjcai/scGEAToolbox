...
%   trainer gets the most blame (false positives)
        ...
        function matrix=computeOverlap(this, sIdSets, tIdSets)
            ...
            for row=1:rows
                ...
                for col=1:cols                
                    ...
                    if matrix(row,col)>0
                        fprintf('%d overap --> %s X %s', matrix(row,col), this.sNames{row}, this.tNames{col});
                        fprintf('\n');
                    end
                end
            end
    ...
    methods(Static)
        ...
        function [names, clrs]=GetMatchingNamesAndColors(qf, lblMap)
            N=length(qf.sIds);
            names=cell(1,N);
            clrs=zeros(N,3);
            for i=1:N
         ...
         function [html, ttl, generated]=DecisionMatrix(QF, showIds, ...
             ...
             if doOverlap
                 if forBrowser
                     html=QF.matrixHtmlOverlap_browser;
                 else
                     html=QF.matrixHtmlOverlap;
                 end
             else
                 if forBrowser
                     html=QF.matrixHtml_browser;
                 else
                     html=QF.matrixHtml;
                 end
             end
             ...
             if doOverlap
                 ...
                 matrixOverlap=matrix;
                 nStudents=length(QF.sNames);
                 nTeachers=length(QF.tNames);
                 SORT_PARKS=true;
                 if sortByOverlap
                     [matrix, rowsI, colsI]=MatBasics.SortDavidParks(matrix);
                 else
                     rowsI=1:nStudents;
                     colsI=1:nTeachers;
                 end
                 [keyRows, keyCols]=MatBasics.Max(matrix);
                 rowHeaders=doHdr(QF.sIds(rowsI), {}, QF.sNames(rowsI), ...
                     QF.sSizes(rowsI), 'test', showIds, matrix);
                 colHeaders=doHdr(QF.tIds(colsI), {}, QF.tNames(colsI), ...
                 ...
                 try
                     ...
                     matrix=QF.matrixOverlap;
                     rowHeaders=doHdr(QF.sIds, {}, QF.sNames, ...
                         QF.sSizes, 'test', showIds, QF.matrixOverlap);
                     colHeaders=doHdr(QF.tIds, {}, QF.tNames, ...
                 ...
                 [keyRows, keyCols]=MatBasics.Min(QF.matrixFinal);
                 ...
                 html=['<html><body bgcolor="white"><center><h2>' ttl '</h2><hr></center>' ...
                     Html.MatrixColored(...
                     doHdr(QF.sIds, QF.sMergedIds, QF.sNames, ...
                     QF.sSizes, 'test', showIds), ...
                     doHdr(QF.tIds, QF.tMergedIds, QF.tNames, ...
                     QF.tSizes, 'train', showIds), QF.matrixFinal,...
         ...
         function [jd, bp, tp]=TextPane(QF, html, args)
            ...
            nS=length(args.sNames);
            nT=length(args.tNames);
            ...
            function LinkCallback(~, e, args, lbl)
                ...
                switch char(et)
                    ...
                    case char(e.getEventType.ACTIVATED)
                        try
                            if idxs(1)==0
                                feval(args.fnc, QF, true, idxs(2))
                            elseif idxs(2)==0
                                feval(args.fnc, QF, false, idxs(1))
                            else
                    ...
                    case char(e.getEventType.ENTERED)
                        try
                            if idxs(1)==0
                                tip=args.sNames{idxs(2)};
                            elseif idxs(2)==0
                                tip=args.tNames{idxs(1)};
                            else
                                try
                                    tip=['row=<b><i>' ...
                                        args.sNames{idxs(1)} ...
                                        '</i></b><br>column=<b><i>' ...
                                        args.tNames{idxs(2)} '</i></b>'];
                                catch %TODO encode mergers
                                    tip='';
                                end
                            end
                            xOffset=mouseEvent.getX+14;
                            yOffset=mouseEvent.getY-5;
                            app.showToolTip(lbl, ['<html>' tip '</html>'],...%);%, ...
                                200, -60, 5);
