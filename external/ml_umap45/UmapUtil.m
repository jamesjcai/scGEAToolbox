...
%   License: BSD 3 clause
    ...
    properties(Constant)
        ...
        METRIC_VALUES={'precomputed', 'euclidean', 'cityblock',...
        'seuclidean', 'chebychev', 'minkowski', 'mahalanobis',...
    ...
    methods(Static)
        ...
        function [args, argued, argsObj]=GetArgs(varargin)
            ...
            if length(varargin)>1 && mod(length(varargin), 2) == 1
                if iscell(varargin{1})
                    varargin=[varargin(1), ...
                        argsObj.parseStr2NumOrLogical(varargin(2:end))];
                else
                    varargin=[varargin{1}, ...
                        argsObj.parseStr2NumOrLogical(varargin(2:end))];
                end
        ...
        function args=CheckArgs(args, argued)
            ...
            if ~isequal(args.label_column, 0) 
                if isempty(args.label_file)
                    if ischar(args.csv_file_or_data) && endsWith(...
                            lower(args.csv_file_or_data), '.csv')
        ...
        function argsObj=SetArgsMetaInfo(argsObj)
            ...
            argsObj.setMetaInfo('mds', 'type', 'logical', ...
                'label', 'Multidimensional scaling');
            argsObj.setMetaInfo('n_neighbors', 'low', 2, 'high', 200, ...
                'is_integer', true, 'label', 'Nearest neighbors');
            argsObj.setMetaInfo('min_dist', 'low', 0.001, 'high', 10);
            argsObj.setMetaInfo('spread', 'low', 0.001, 'high', 10);
            ...
            argsObj.setMetaInfo('n_components', 'label', ...
                '# reduced dimensions', 'low', 2, 'high', 10,...
                'is_integer', true, 'text_columns', 2);
            ...
            argsObj.setArgGroup({'match_table_fig', ...
                    'match_histogram_figs', 'qf_tree', 'mds'}, ...
                    'Match settings');
        ...
        function p=DefineArgs
            ...
            addParameter(p,'mlp_supervise', false, @islogical);
            addParameter(p,'mlp_only', false, @islogical);
            ...
            addParameter(p, 'template_folder', '', @Args.IsFolderOk);
            ...
            addParameter(p, 'flowjo_columns', [], @(x)ischar(x) ...
                || (iscell(x) && ischar(x{1})));
            addParameter(p, 'flowjo_ask', true, @islogical);
            %These arguments are given only by fcs/SuhGating files
            addParameter(p, 'flowjo_wsp', [], @(x)isa(x, 'FlowJoWsp'));
            addParameter(p, 'flowjo_tree', [], @(x)isa(x, 'FlowJoTree'));
            addParameter(p, 'gates', [], @(x)iscell(x) && ...
                ~isempty(x) && isa(x{1}, 'SuhGate'));
            
            
            %next 2 are set by FlowJoTree for highlighting purposes
            addParameter(p, 'highlighter_registry', [], @validateCallback);
            addParameter(p, 'sample_rows', [], @islogical);
            addParameter(p, 'sample_offsets', [], @(x)isa(x, 'Map'));
            addParameter(p, 'conclude', [], @(x)isa(x, 'function_handle'));
            addParameter(p, 'save_roi', [], @(x)isa(x, 'function_handle'));
            addParameter(p, 'mds', false, @islogical);
            addParameter(p, 'reduction_algorithm', 'UMAP', ...
                @(x) any(validatestring(x, {'UMAP', 'PHATE'}))); 
            addParameter(p, 'args_phate', [], ...
                @(x)~isempty(x) && isstruct(x) && isfield(x, 'k')); 
            addParameter(p, 'landmarks', 600, @(x)isnumeric(x) ...
                && (x>-10 && x<=12000 ));
            addParameter(p, 'ignoreScatter', true, @islogical);

            % work around FlowJo scaling
            % issue described by Dylan Hinson
            addParameter(p, 'rescale', 1, @(x) x>0 && x<100000);
            addParameter(p, 'rescale_nudge', 3, @(x) x>0 && x<10000);
            addParameter(p, 'jiggle_probability_bin', 0.0066, @jiggleOk);
            addParameter(p, 'maxDeviantParameters', 0, @(x)isnumeric(x) && x>=-1 && x<20);
            addParameter(p, 'robustConcordance', false, @islogical);

            function ok=jiggleOk(x)
                if ischar(x)
                    x=str2double(x);
                end
        ...
        function [qft, tNames]=Match(args, unreducedData, tLbls, tLblMap,...
            ...
            if predictions
                ...
                listener.explorerName='Parameter'; %for window title
       ...
       function sc=GetMatchScenarioText(scenario, reductionType)
        ...
        function pths=SetPaths
            pth=fileparts(mfilename('fullpath'));
            utilPath=fullfile(fileparts(pth), 'util');
            existingPaths = strsplit(path, pathsep);
            if ~ismember(pth, existingPaths)
                addpath(pth);
            end
            ...
            FileBasics.AddNonConflictingPaths(pths);
        end

        function [args, argued]=Initialize(varargin)
            if ~isdeployed
                pPth=fileparts(fileparts(mfilename('fullpath')));
            ...
            args.phate=~startsWith(lower(args.reduction_algorithm), 'u');
            if args.phate
                if ~PhateUtil.IsInstalled
                    exc= MException('UMAP:PHATE', ...
                        'PHATE needs to be installed.');
                    throw(exc);
                end
                args.reduction_algorithm='PHATE';
            else
        ...
        function args=SaveUmapArgs(args, fileName)
            args.unreduced_data=[]; % too big
            args.save_roi=[];
            args.gates=[];
            args.flowjo_wsp=[];
            args.flowjo_tree=[];
            args.highlighter_registry=[];
            args.locate_fig=[];
            args.conclude=[];
            if nargin>1
                if ischar(fileName)
                    save(fileName, "args");
                elseif islogical(fileName) && fileName
                    save(File.Downloads('lastUmapArgs'), "args");
                    msg(['<html>'  Html.H2('Saved UMAP args') ...
                        Html.FileTree(File.Downloads('foo')) '</html>'])
                end
            end
        end

        function args=LoadUmapArgs(fileName)
            load(fileName, "args");
        end

        function failed=CompareUmapArgs(args1, verbose, args2)
            args1=UmapUtil.SaveUmapArgs(args1);
            if nargin < 3
                args2=UmapUtil.LoadUmapArgs( ...
                    File.Downloads('lastUmapArgs'));
                if nargin < 2
                    verbose=true;
                end
            elseif ischar(args2)
            ...
            ok=isequal(args1, args2);
            flds=fields(args1);
            N=length(flds);
            failed={};
            if verbose
                fprintf(['Overall equality=%d, checking each ' ...
                    'structure field...'], ok);
            end
            for i=1:N
                name=flds{i};
