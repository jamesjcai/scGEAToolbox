...
%   This wrapper around MATLAB internal map is meant for polymorphic
%   function signature compatability with other classdef and Java class in
%   CytoGenie AutoGate
...
%   License: BSD 3 clause
    ...
    methods(Static)        
        ...
        function obj=SetHighDef(obj, hasHighDef)
            ...
            if factor>0
                ...
                obj.smallEnd='</font>';
                obj.supStart=['<sup>' obj.smallStart];
                obj.supEnd=[obj.smallEnd '</sup>'];
                obj.subStart=['<sub>' obj.smallStart];
                obj.subEnd=[obj.smallEnd '</sub>'];
    ...
    methods
        function this=BasicMap(keysOrFileName, values)
            ...
            this.counts=edu.stanford.facs.swing.Counter;
        ...
        function oldCbn=setColorsByName(this, cbn_)
            oldCbn=this.cbn;
            this.cbn=cbn_;
        end
        
        function base=adjustHighDef(this, base, additional)
            if this.highDef
                if nargin==2
                    base=(this.toolBarFactor-1)*base;
                else
                    base=base+((this.toolBarFactor-1)*additional);
                end
            end
        end
            ...
            function show(nArgs)
                ...
                if nArgs>=5
                    ...
                    if ~isempty(txt)
                        ...
                        javaMethodEDT('showLater', this.ttod, cmp, false,...
                            bottomCmp, xOffset, yOffset, hideCancelButton);
                    end
                elseif nArgs>=3
        ...
        function v=VERSION(this) %uppercase for CytoGate compatibility
            v=this.getVersion;
        end
        ...
        function setAppVersion(this, autoGateV, umapV, eppV)
            ...
            if ~isempty(autoGateV)
                ...
                this.remoteSourceCode='GetDown2/domains/FACS/AutoGate.zip';
                this.sourceStartCmd='main';
            else
                this.sourceStartCmd='fjb or suh_piplines';
        ...
        function [sourceVersionFile, remoteFolder, f, ext]=getSourceVersionFile(this)
            [remoteFolder,f, ext]=fileparts(this.remoteSourceCode);
            sourceVersionFile=[f 'Version.txt'];
        end
        ...
        function cnt=Tally(this, key)
            this.counts.count(key);
            cnt=this.counts.getCount(key);
        end

        function ok=areSoundsOn(this)
            ok=this.is('soundsOn', true);
        end
        
        function was=setSoundsOn(this, yes)
            was=this.areSoundsOn;
            if yes
                v='yes';
            else
                v='no';
