...
%   License: BSD 3 clause
    ...
    properties(Constant)
        ...
        COLOR_BORDER_UNSELECTED=[.5 0 .65];
        COLOR_BORDER_SELECTED=[.5 .52 0];
    ...
    methods
        ...
        function [displayData, display2Event, displayWeight]=getDisplayData(this, data)
            ...
            if nargin==1 
                ...
                binIdxsWithWeightButNotEvents=find(~ismember(...
                    binIdxsWithWeight, binIdxsWithEvents));
                N2=length(binIdxsWithWeightButNotEvents);
                for i=1:N2
        ...
        function y=getY(this)
            [ygrid_,xgrid_]=meshgrid(this.ye(2,:), this.ye(1,:));
            y=[xgrid_(:) ygrid_(:)]';
        end
    ...
    methods(Static)
        ...
        function [numClusters, clusterIds, density]=GetClusters(...
                data, detail, mins, maxs)
            if nargin<4
                maxs=[1 1];
                if nargin<3
                    mins=[0 0];
            ...
            [numClusters, clusterIds, density]=...
                Density.FindClusters(...
                data, detail, 'dbm', ...
                [], [], [], [], ... %ignore dbscan arguments
                mins, maxs);
        end
        ...
        function [numClusters, clusterIds, density]=FindClusters(...
            ...
            app=BasicMap.Global;
            canNotDo=app.noDbscan || ((MatBasics.usingOlderThan('R2019a') ... 
            ...
            if wantsDbscan && ~canNotDo
                    ...
                    try
                        ...
                        if ~isdeployed && ~isempty(pu) && ~isempty(pu2)
                            msg(Html.WrapHr(['Density:FindClusters '...
                                ...
                                '<br>from MathWorks File Exchange<br>'...
                ...
                density.numClusters=numClusters;
            end
            if wantsDbscan && canNotDo
                wantsDbscan=false;
        ...
        function dtls=DetailsHtml(app)
            ...
            N=length(dtls);
            for i=1:N
                dtls{i}=Html.WrapSmallBold(dtls{i}, app);
            end
        end
    ...
    methods
        function [numClusts, eventClusterIds]=clusterAnalyze(this, options)
            ...
            if slopeIsSignificant ~= 0
                ...
                %Vectors normalized in euclidean norm
            ...
            if debugNewJavaMerging>0
                ...
                writematrix(f, 'density.txt');
                writematrix(Pointers, 'pointers.txt');
                writematrix(stdErr, 'stdErr.txt');
                javaMerging=tic;
            end
            noJava=false;
            try
                    ...
                    for i = 1:numDummies
                        ...
                        if debugNewJavaMerging>0
                            ...
                            if useOldMerging
                                ...
                                debug_=B(yQ);
                                assert(Pointers(debug_)==newPeak);
                            end
                        end
            ...
            eventClusterIds=uint64(eventClusterIds);
            this.numClusters=numClusts;
        end
        
        function [eventClusterIds, pointersWithEvents]=getEventData(this)
            clusts=unique(this.pointers);
            clusts=clusts(clusts~=0);  %all non-background clusters            
            j=1;
            for i=clusts
                this.pointers(this.pointers==i)=j;  %relabel pointers
            ...
            if nargout>1
                if isempty(eventClusterIds)
                    ...
                    pointersWithEvents=zeros(1, this.M^2);
                    pointersWithEvents(this.eventBinIdxs)=eventClusterIds;
                end
            end
            if this.truncated
                ca=zeros(1, length(this.onScale));
        ...
        function xy=getPolygon(this, clusterIds,...
                onlyBinsWithEvents, tolerance)
            if nargin<4 
                tolerance=1;
                if nargin<3
                    onlyBinsWithEvents=false;
                end
            end
            if onlyBinsWithEvents
                bi=find(ismember(this.pointersWithEvents, clusterIds));
            else
                bi=find(ismember(this.pointers, clusterIds));
            end
            [x,y]=ind2sub([this.M this.M], bi);
            xy=[x;y]';
            xy=xy(boundary(xy), :);
            xy=double(edu.stanford.facs.swing.CurveSimplifier.ToArray(...
                xy, tolerance));
            [x, y]=this.toData(xy(:,1), xy(:,2));
            xy=[x y];
        end

        function [x, y]=getPolygonForRegion(this, xy, tolerance)
            if nargin<3
                tolerance=1;
            end
            xy=xy(boundary(xy), :);
        ...
        function clueCnts=getClusterCounts(this)
            x=this.pointersWithEvents(this.eventBinIdxs);
            lbls=unique(x);
            clueCnts=MatBasics.HistCounts(x, lbls);
            if any(lbls<1)
                clueCnts=clueCnts(lbls>0);
            end
        end
    ...
    methods(Access=private)
        function this=Density(events, options, mins_, maxs_)
            ...
            %PUBREF=REF 2.2
            %Java equivalent of conv2 is
            %c=c=edu.stanford.facs.swing.Convolution();
            %ccc=c.convolution2DPadded(wmat_, 256, 256, this.phimat, 47, 47)
    ...
    methods(Static)
        ...
        function Debug(M)
            ...
            xSpace=linspace(8000,9000, M);
            ySpace=linspace(2000,3000, M);
            wMatrix=reshape(1:M^2, M,M);
            ...
            assert(isequal(x, xx) && isequal(y, yy));
            for i=1:length(xx)
        ...
        function ok=ValidateDetail(x)
            ...
            elseif iscell(x)
                ...
                for i=1:N
                    ...
                    if idx<1
                        disp(['Unrecognized cluster detail "' v '"']);
                        ok=false;
                        break;
                    end
                end
        ...
        function btn=ButtonPick(tb, callback, roiName)
            if nargin<3
                roiName='gate';
            end
            btn=ToolBarMethods.addButton(tb, ...
                        'pickGate.png', ...
                        ['Create ' roiName ' with cluster pick'], ...
                        callback);
        end
        
        function [btn, combo, lbl, pnl]...
            =ComboDetail(callbackCluster, tb, callbackCreate, app)
            if nargin<3
                app=BasicMap.Global;
                if nargin<2
                    tb=[];
                    if nargin<1
                        callbackCluster=@(h,e)fprintf(...
                            'Detail="%s", index=%d\n',...
                            String.RemoveXml(...
                            h.getSelectedItem),...
                            h.getSelectedIndex);
            ...
            drawnow;
            dtls=Density.DetailsHtml(app);
            dtls(end)=[];
            items=[Html.WrapSmallBold('none', app), dtls];
            btn=Density.ButtonPick( ...
                tb, callbackCreate, 'region of interest');
            lbl=Gui.Label(Html.WrapSm('Cluster ', app));
            lbl.setToolTipText(btn.getToolTipText)
            pb=handle(lbl,'CallbackProperties');
            if ~isempty(callbackCreate)
                set(pb, 'MousePressedCallback',callbackCreate)
            else
                btn.setEnabled(false);
                btn.setToolTipText([]);
                lbl.setToolTipText([]);
            end            
            combo=Gui.Combo(items, 0, '',[],callbackCluster, ...
                'Select level of clustering detail');
            pnl=Gui.FlowLeftPanelBorder(btn, lbl, combo);
            if ~isempty(tb)
                ToolBarMethods.addComponent(tb, pnl);
                pnl.setVisible(true);
                drawnow;
            end
        end

        function jd=MakePolgygonMsg(callback, parentFig, where)
            if nargin<3
                where='north east+';
                if nargin<2
            ...
            stop=Html.WrapSmall([ Html.ImgXy('close.gif', [], 1.21) ...
                '&nbsp;<font color="red">Stop </font>']);
            convert=[Html.Img('pickGate.png') '&nbsp;' ...
                Html.Img('rightArrow.png') '&nbsp' ...
                Html.Img('polygonGate.png')];
            make=Html.WrapSm(convert);
            [~, ~, ~, ~, jd]=questDlg(struct('modal', false, ...
                'where', where, 'msg', Html.WrapSm( ['Pick 1 or'...
                ' more clusters then <b>click</b> ' convert]), ...
                'checkFnc', @choiceMade, ...
                'icon', 'none'), 'Convert cluster to polygon?', ...
                make, stop, make);
            SuhWindow.Follow(jd, parentFig, where, true);
            btn=Gui.NewBtn('.', @(h,e)nothing());
            Gui.RegisterEscape(jd.getRootPane, btn);

            function nothing()
                disp('Escape cannot close cluster-to-polygon window');
            end
            function ok=choiceMade(jd, finalAnsw)
                ok=feval(callback, strcmpi(make, finalAnsw), jd);
            end
        end

        function UnselectBorders(hBorders)
            N_=length(hBorders);
            for i=1:N_
                set(hBorders(i), 'Color', Density.COLOR_BORDER_UNSELECTED);
            end
        end
    end 
    
    %GUI support properties&methods
    properties
        detail;
        method;
        borders;
    ...
    methods
         function [clust_id, xBin, yBin, bi]=getCluster(this, x_click, y_click)
            if x_click>=this.mins(1) && x_click<=this.maxs(1) &&...
                    y_click>=this.mins(2) && y_click<=this.maxs(2)
                bi=this.toBinIdxs(y_click, x_click);
                [xBin, yBin]=ind2sub([this.M this.M], bi);
                if bi < length(this.pointers) && bi>0
                    clust_id=this.pointers(bi);
                else
                    if bi>0 && bi<length(this.pointersWithEvents)
                        clust_id=this.pointersWithEvents(bi);
                    else
                        clust_id=0;
                    end
                end
         ...
         function xy=getData(this, clusterIds)
            [x,y]=ind2sub([this.M this.M], find(ismember(...
                this.pointersWithEvents, clusterIds)));
            [x, y]=this.toData(x, y);
            xy=[x;y]';
         end

         function [roi, clue, strXy]=getROI(this, ax, hBorders, callBack, ...
                 tolerance)
             if nargin<5
                 tolerance=1;
                 if nargin<4
             ...
             clue=this.getCluster(cp(1,1),cp(1,2));
             N_=length(hBorders);
             if (clue>0 && clue<=N_)
                 if isequal(get(hBorders(clue), 'Color'), Density.COLOR_BORDER_SELECTED)
                     set(hBorders(clue), 'Color',  Density.COLOR_BORDER_UNSELECTED);
                 else
                     set(hBorders(clue), 'Color',  Density.COLOR_BORDER_SELECTED);
                 end
             end
             clusterIds=[];
             for clue=1:N_
                 if isequal(get(hBorders(clue), 'Color'), Density.COLOR_BORDER_SELECTED)
                     clusterIds(end+1)=clue;
                 end
             end
             if isempty(clusterIds)
                 ...
                 [roi, strXy]=RoiUtil.NewPolygon(ax, ...
                     this.getPolygon(clusterIds, false, tolerance),...
                     callBack);
             end
         end
         
         function Hs=drawBorders(this, ax, clr, markerSize)
            if nargin<4
                markerSize=.8;
                if nargin<3
                    clr=[];
            ...
            if isempty(clr)
                clr=Density.COLOR_BORDER_UNSELECTED;
            end
            wasHeld=ishold(ax);
            if ~wasHeld
                hold(ax, 'on');
            ...
            Hs=zeros(1,N_);
            for i=1:N_
                Hs(i)=gridEdge(this, true, i, clr, ax, ...
                    markerSize, '.', '-', .5);
                if Supervisors.VERBOSE
                    str2double(this.clusterColors{i})
            ...
            this.borders=Hs;
            try
                Gui.StretchLims(ax, Hs, .02);
            catch ex
                ex.getReport
            end
          end
        
         function removeBorders(this)
            if ~isempty(this)
                Hs=this.borders;
                this.borders=[];
                N_=length(Hs);
