classdef CRX < quantum.internal.gate.gates.ControlledQubitGate
    ...
    methods
        function obj = CRX(controlQubit, targetQubit, theta)
            obj@quantum.internal.gate.gates.ControlledQubitGate(controlQubit, targetQubit);
            obj.gateMatrix = [cos(theta/2), -1i*sin(theta/2);...
                -1i*sin(theta/2), cos(theta/2)];
            obj.theta = theta;
        end

        function obj = inv(obj)
            obj = quantum.internal.gate.gates.CRX(obj.controlQubit, obj.targetQubit, -obj.theta);
        end
    end
