classdef RYY < quantum.internal.gate.gates.TwoQubitRotationGate
    ...
    methods
        function obj = RYY(targetQubit1, targetQubit2, theta)
        ...
        function obj = inv(obj)
            obj = quantum.internal.gate.gates.RYY(obj.targetQubit1, obj.targetQubit2, -obj.theta);
        end
    end
    ...
    methods(Hidden)
        function [instr, def, map] = getQASM(obj, Unpack, map, SupportedGates, varargin)
            ...
            if isKey(SupportedGates, obj.type)
                ...
                gates = [rxGate(1,pi/2) rxGate(2,pi/2) cxGate(1,2) rzGate(2,obj.theta) cxGate(1,2) rxGate(1,-pi/2) rxGate(2,-pi/2)];
