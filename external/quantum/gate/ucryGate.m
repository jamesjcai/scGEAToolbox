function cg = ucryGate(controlQubits, targetQubit, angles, NameValueArgs)
%UCRYGATE Uniform controlled Y-axis rotation gate
%
%   cg = UCRYGATE(controlQubits, targetQubit, angles) returns a CompositeGate
%   that applies a Y-axis rotation to a target qubit for each computational
...
%
%   cg = UCRYGATE(___, RotationThreshold=THRES) removes rotation gates
...
%   is block-diagonal, where ryMi represents the 2x2 Y-axis rotation matrix
...
%                         [  0        ryMn]
%
%   See also compositeGate, unitaryGate, initGate, ucrzGate, ucrxGate
...
if Nc==0
    g = ryGate(targetQubit, angles);
    cg = quantum.internal.gate.filterRotationGates(g, thres);
else
    gates = quantum.internal.gate.ucrgates(angles, @ryGate, @cxGate, thres);
    cg = compositeGate(gates, [controlQubits targetQubit], Name="ucry");
end
end