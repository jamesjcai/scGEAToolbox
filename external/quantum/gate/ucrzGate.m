function cg = ucrzGate(controlQubits, targetQubit, angles, NameValueArgs)
%UCRZGATE Uniform controlled Z-axis rotation gate
%
%   cg = UCRZGATE(controlQubits, targetQubit, angles) returns a CompositeGate
%   that applies a Z-axis rotation to a target qubit for each computational
...
%
%   cg = UCRZGATE(___, RotationThreshold=THRES) removes rotation gates
...
%   is diagonal, where rzMi represents the 2x2 Z-axis rotation matrix
...
%                         [  0        rzMn]
%
%   See also compositeGate, unitaryGate, initGate, ucrxGate, ucryGate
...
if Nc==0
    g = rzGate(targetQubit, angles);
    cg = quantum.internal.gate.filterRotationGates(g, thres);
else
    gates = quantum.internal.gate.ucrgates(angles, @rzGate, @cxGate, thres);
    cg = compositeGate(gates, [controlQubits targetQubit], Name="ucrz");
end
end