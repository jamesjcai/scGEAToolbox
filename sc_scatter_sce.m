...
i_addmenu(m_ext,0,@RunSeuratWorkflow,'Run Seurat/R Workflow (Seurat/R)...');
i_addmenu(m_ext,0,@callback_TrajectoryAnalysis,'Pseudotime Analysis (Monocle/R)...');
i_addmenu(m_ext,1,@callback_MELDPerturbationScore,'MELD Perturbation Score (MELD/Python) [PMID:33558698]...');
i_addmenu(m_ext,0,{@SubsampleCells,2},'Geometric Sketching (geosketch/Python) [PMID:31176620]...');
i_addmenu(m_ext,0,@HarmonyPy,'Batch Integration (Harmony/Python) [PMID:31740819]...');
i_addmenu(m_ext,0,@DoubletDetection,'Detect Doublets (Scrublet/Python) [PMID:30954476]...');
...
%         if ~strcmp(answer,'Yes'), return; end
    ...
    function SubsampleCells(src,~,methodoption)
        ...
        elseif  methodoption==2
            uiwait(helpdlg(['"Large-scale single-cell RNA sequencing (scRNA-seq) ' ...
                ...
                ' cell types via clustering."'],'Geometric Sketching [PMID:31176620]'));
...
uiwait(helpdlg(['Droplet-based microfluidic devices have become widely ' ...
    ...
    'into scRNA-seq workflows to improve downstream analyses.'],'DecontX [PMID:32138770]'));
    ...
    function HarmonyPy(src, ~)
        ...
        uiwait(helpdlg(['The emerging diversity of single-cell RNA-seq datasets ' ...
    ...
    function DoubletDetection(src, ~)
        ...
        uiwait(helpdlg(['ScRNA-seq experiments generate ''multiplet'' errors when ' ...
            ' multiple cells are labeled with the same barcode. ' ...
            'Wolock et al. describe Scrublet, a method for predicting ' ...
            'the effects of multiplets on downstream analyses and identifying problematic multiplets.'], ...
            'Scrublet [PMID:30954476]'));
