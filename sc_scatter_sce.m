...
m.Accelerator = 'x';
m2 = uimenu(m,'Text','sc&Tenifold','Accelerator','T');
uimenu(m2,'Text','scTenifoldNet Network Construction 🐢🐢 ...',...
    'Callback',@callback_scTenifoldNet1);
uimenu(m2,'Text','scTenifoldNet Network Comparison 🐢🐢🐢 ...',...
    'Callback',@callback_scTenifoldNet2);
% uimenu(m2,'Text','---------------------------------------');
uimenu(m2,'Text','scTenifoldKnk Virtual KO Analysis...',...
    'Separator','on',...
    'Callback',@callback_scTenifoldKnk);
...
switch ButtonName
    ...
    function SelectCellsByQC(src, ~)
        oldn=sce.NumCells;
        oldm=sce.NumGenes;
        [requirerefresh,highlightindex]=gui.callback_SelectCellsByQC2(src);
        sce = guidata(FigureHandle);
        if requirerefresh            
            [c, cL] = grp2idx(sce.c);
