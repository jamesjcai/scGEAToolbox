function [Tdv_final, Tig, influgenes] = sc_variability( X, X2, genelist, genelist2, fname, plotit )
    ...
    clear genelist3  irows jrows T T2;
    ...
    [~,dist] = dsearchn(xyzp, xyz);

    Tdv = table(genelist, dist);
    Tdv.Properties.VariableNames = {'genes', 'dist'};
    [Tdv, idx] = sortrows(Tdv,'genes','descend');
    genelist = genelist(idx);
    xyz = xyz(idx,:);
    xyzp = xyzp(idx,:); % Probably not needed
    clear dist;

    % Position vector referenced at polynomial
    [~,dist2] = dsearchn(xyzp2, xyz2);
    ...
    npolyg_idx = dsearchn(xyz(:,1:3), xyzp(idx,1:3));

    % Genes for largest polynomial distances
    polygenes = genelist(npolyg_idx);
