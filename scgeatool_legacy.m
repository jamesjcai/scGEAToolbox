...
%SCGEATOOL_LEGACY - Single-cell Gene Expression Analysis Toolbox Graphical User Interface.
...
% ------------------------
    ...
    function in_addbuttonpush(toolbarHdl, sepTag, callbackFnc, imgFil, tooltipTxt)
        ...
        if numel(ptImgCell) >= ptimgidx && ~isempty(ptImgCell{1, ptimgidx})
            gui.i_addbutton2fig(barhandle, sepTag, ...
                callbackFnc, ptImgCell{1, ptimgidx}, tooltipTxt);
        else
            [~, ptImgCell{1, ptimgidx}] = gui.i_addbutton2fig(barhandle, ...
